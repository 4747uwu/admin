...
const User = (props: any) => {
    ...
    const handleNotify = (id: any) => {
        dispatch(
            openDialog({ type: "notification", data: { id, type: "user" } })
        );
    };
    ...
    const userTable = [
...

        {
            ...
            Cell: ({ row }: { row: any }) => (
                <span className="text-capitalize fw-normal">{row?.rechargedCoins || 0}</span>
            ),
        },

        {
            Header: "Following",
            Cell: ({ row }: { row: any }) => (
                <span className="text-capitalize fw-normal">{row?.totalFollowings || 0}</span>
            ),
        },
...

        {
            Header: "IsHost",
            Cell: ({ row }: { row: any }) => (
                <span className="text-capitalize fw-normal">{row?.isHost === true ? "Yes" : "No"}</span>
            ),
        },

        {
            Header: "IsVip",
            Cell: ({ row }: { row: any }) => (
                <span className="text-capitalize fw-normal">{row?.isVip === true ? "Yes" : "No"}</span>
            ),
        },
            ...
            <div className="d-flex justify-content-between align-items-center">
                ...
                <div className="col-6 mt-3">
                    <Searching
                        ...
                        placeholder={"Search by User Name/UniqueID"}
                    />
                </div>
            </div>
